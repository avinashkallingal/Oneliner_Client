...
import { useState, useEffect } from 'react';
import axiosInstance from '../../../Constarints/axios/adminAxios';
import { Card, CardContent, Typography, Box, Grid, Button, CardMedia } from '@mui/material';
import { toast } from 'sonner';
import { useNavigate } from 'react-router-dom';
import { postEndpoints } from '../../../Constarints/endpoints/postEndpoints';
      ...
      <Grid container spacing={3} justifyContent="center" alignItems="center" flexGrow={1}>
        {currentPosts.map((post, index) => (
          <Grid item xs={12} sm={6} md={4} key={index}>
            <Card sx={{ height: '100%' }}>
              {/* Post Image */}
              {post.imageUrlS3 && post.imageUrlS3[0] && (
                <CardMedia
                  component="img"
                  alt="Post Image"
                  height="140"
                  image={post.imageUrlS3[0]} // Assuming the first image in the array is what you want to use
                />
              )}
              <CardContent>
                <Typography variant="h6" gutterBottom>
